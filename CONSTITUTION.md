...
Bump rationale: PATCH. No principle is added, removed, or redefined. The Development
  Workflow and Quality Gates section named spec-kit as the workflow tool; the project now
  works through OpenSpec, so the section is retargeted and the gate is restated in terms of
  the artifacts that now carry it. The gate itself is unchanged: the Constitution Check runs
  before the work starts and again after the design settles, and a violation carried forward
  is recorded with the rejected simpler alternative.

Modified sections:
  Development Workflow and Quality Gates — spec-kit command flow replaced with the OpenSpec
    flow; "the plan's Complexity Tracking table" replaced with "the change's design.md under
    Risks / Trade-offs", which is where that record now lives.

Unmodified principles: I, II, III, IV, V

File relocation (not an amendment): this document moved from .specify/memory/constitution.md
  to CONSTITUTION.md at the repository root when the spec-kit scaffolding was removed. Its
  content and standing are unchanged.

Template consistency:
  ✅ AGENTS.md, CLAUDE.md, README.md — workflow sections and constitution links updated in
     the same change.
  ✅ openspec/changes/venue-session-state/ — the migrated slice records its Constitution
     Check in design.md; gate results are preserved verbatim from the spec-kit plan.
...
- Work flows through OpenSpec: a change proposes requirement deltas, the deltas are
  implemented, and archiving the change folds them into `openspec/specs/` as current truth.
  Constitution Check gates on the principles above and MUST pass before research begins and
  again after the design settles.
- Any Constitution Check violation carried forward MUST be recorded in the change's
  `design.md`, under Risks / Trade-offs, with the simpler alternative that was rejected and
  why.
...
**Version**: 1.2.1 | **Ratified**: 2026-07-29 | **Last Amended**: 2026-07-30
