---
description: "Task list for 001-venue-session-state"
---

# Tasks: Venue Session State

**Input**: Design documents from `specs/001-venue-session-state/`

**Prerequisites**: plan.md, spec.md, research.md, data-model.md, contracts/, quickstart.md — all present.

**Tests**: **MANDATORY, not optional.** Constitution v1.2.0 Principle V (Test-First with Golden
Vectors) is NON-NEGOTIABLE and supersedes the template's default that tests are optional. Tests
are written first and must FAIL before implementation. Vectors are permanent and are never
...
Cargo workspace per plan.md Structure Decision:

- `crates/market-time-core/` — pure; no I/O, no network, no clock
- `crates/market-time-scales/` — the hifitime bridge, the one place the D1a seam lives
- `crates/market-time-data/` — dataset revisions and loaders; all I/O lives here
- `crates/market-time-cli/` — thin shell
- `crates/market-time-board/` — thin shell, the global board
...
- [ ] T007 Decide and record the phase boundary ownership convention (which side of an edge is inclusive) in `specs/001-venue-session-state/data-model.md` under a new "Boundary ownership" section. No source document settles this; it is ours to decide, and FR-004 requires every instant to belong to exactly one phase. Carried-forward blocker #7
...
- [ ] T019 Add a dependency guard test in `crates/market-time-core/tests/contract/no_io_no_clock.rs` asserting that `market-time-core`'s dependency graph contains nothing capable of I/O, networking, or a clock read, so a Principle IV violation fails the build rather than failing review
- [ ] T020 Build the golden vector harness in `crates/market-time-core/tests/vectors/mod.rs`: vectors are data files, are enumerated, and the harness fails if a vector file is removed (Principle V — vectors are permanent)

**Checkpoint**: type system and invariants in place; user stories can begin.

---

## Phase 2V: Source Verification (blocks data, runs parallel with Phase 2)
...
## Phase 3: User Story 1 — Phase at an instant (Priority: P1) 🎯 MVP
...
- [ ] T026 [P] [US1] SSE vectors in `crates/market-time-core/tests/vectors/sse.rs`: continuous trading, mid-day break, opening call auction, closing call auction, after-hours fixed price, Chinese public holiday (quickstart SSE-1..SSE-6)
- [ ] T027 [P] [US1] NYSE vectors in `crates/market-time-core/tests/vectors/nyse.rs`: pre-market, core session, after-hours, US holiday, early close (quickstart NYSE-1..NYSE-5)
- [ ] T028 [P] [US1] NYSE DST vectors in `crates/market-time-core/tests/vectors/nyse_dst.rs`: **spring-forward (a local time that does not exist)** and **fall-back (a local time occurring twice)**. Assert `jiff`'s `Disambiguation::Reject` behaviour against real instants rather than trusting the documentation — blocker #6 (quickstart NYSE-6, NYSE-7)
- [ ] T029 [P] [US1] Binance vectors in `crates/market-time-core/tests/vectors/binance.rs`: normal continuous instant, funding boundary, announced maintenance window (quickstart BIN-1, BIN-2, BIN-5)
- [ ] T030 [P] [US1] Cross-venue boundary and coverage vectors in `crates/market-time-core/tests/vectors/boundaries.rs`: an instant exactly on a phase edge, and an instant outside declared coverage for each venue (FR-002, FR-004)
- [ ] T031 [P] [US1] Contract test for `resolve_phase` in `crates/market-time-core/tests/contract/resolve.rs` per contracts/core-api.md: `at: UtcInstant` is a mandatory first parameter, and the returned answer carries phase, boundaries, uncertainty, evidence, and dataset revisions
...
- [ ] T039 [US1] Implement `--at now` in `crates/market-time-cli/src/main.rs` such that the clock is read in the shell, passed into the core, **and the host clock discipline bound is surfaced as uncertainty** (constitution, Domain and Data Constraints)
- [ ] T040 [US1] Build the SSE dataset revision in `data/sse/` with per-rule evidence and declared coverage (gated on T021 — the two unassigned intervals must be resolved from source first)
- [ ] T041 [US1] Build the NYSE dataset revision in `data/nyse/` with per-rule evidence and declared coverage (gated on T022 for terms, T024 for the session table)
- [ ] T042 [US1] Build the Binance dataset revision in `data/binance/` including the **variable** funding recurrence — 8-hourly default, 4-hourly for the named contract subset, hourly under volatility, with reversion thresholds that themselves changed over time. A fixed "every 8 hours" field would be wrong (research D5; gated on T023)

**Checkpoint**: User Story 1 fully functional and independently testable. **This is the MVP.**

---

## Phase 4: User Story 2 — Evidence behind the answer (Priority: P2)
...
- [ ] T043 [P] [US2] Evidence-completeness test in `crates/market-time-core/tests/contract/evidence.rs`: no answer may be returned without at least one openable source reference (SC-002, SC-008)
- [ ] T044 [P] [US2] Derived-marking test in `crates/market-time-core/tests/contract/derived.rs`: a rule with `is_derived` true and empty `derivation_reasoning` MUST fail validation (FR-010)
- [ ] T045 [P] [US2] Uncertainty-floor vector in `crates/market-time-core/tests/vectors/uncertainty.rs`: a funding event's stated uncertainty MUST be no narrower than Binance's own published 15-second deviation (FR-011a, SC-007, research D5a)
- [ ] T046 [P] [US2] Process-boundary vector in `crates/market-time-core/tests/vectors/process_boundary.rs`: NYSE's 09:30 MUST carry a spread rather than being presented as an instantaneous market-wide transition (FR-011b, research D3)
- [ ] T047 [P] [US2] Reproducibility test in `crates/market-time-core/tests/contract/reproducible.rs`: the same query against unchanged dataset revisions returns an identical answer (FR-017, SC-005)
...
## Phase 5: User Story 3 — The global board (Priority: P3)
...
- [ ] T053 [P] [US3] Multi-venue contract test in `crates/market-time-core/tests/contract/multi_venue.rs`: `resolve_phases` returns a per-venue vector with no batch-level error path (FR-019)
- [ ] T054 [P] [US3] Partial-unknown test in `crates/market-time-core/tests/contract/partial_unknown.rs`: one venue outside coverage reports unknown while the others answer normally (FR-020, SC-004)
- [ ] T055 [P] [US3] Board clock-discipline test in `crates/market-time-board/tests/clock_discipline.rs`: a board displaying "as of now" exposes the host's discipline bounds, and when discipline data is unavailable it MUST NOT fall back to presenting the clock as exact
- [ ] T056 [P] [US3] Vocabulary-purity test in `crates/market-time-core/tests/contract/vocabulary.rs`: all three venues are expressed entirely in the shared phase vocabulary, zero venue-specific names (FR-005, SC-006)

### Implementation for User Story 3

- [ ] T057 [US3] Implement `resolve_phases` in `crates/market-time-core/src/resolve.rs` (FR-019, FR-020) (depends on T032)
- [ ] T058 [US3] Implement the board's single per-render `resolve_phases` call in `crates/market-time-board/src/lib.rs` — **one `now`, shared across all venue tiles**, so the snapshot is coherent (contracts/board.md)
- [ ] T059 [US3] Render venue tiles in `crates/market-time-board/src/tile.rs`, with unknown **visually distinct from closed** — they are different claims (contracts/board.md)
- [ ] T060 [US3] Render uncertainty in `crates/market-time-board/src/uncertainty_view.rs`. Governing rule from spec.md: if a display cannot express an honest answer, **the display changes, not the answer**
- [ ] T061 [US3] Render the "as of" line in `crates/market-time-board/src/header.rs` carrying the host clock discipline bound (depends on T055)
...
- [ ] T062 Run the full quickstart validation in `specs/001-venue-session-state/quickstart.md` and record results
- [ ] T063 [P] Write `README.md` usage section now that a real interface exists (it currently states pre-alpha with no released code)
- [ ] T064 [P] Record each venue's licensing tier and its evidence in `DATA-LICENSING.md`
- [ ] T065 [P] Add `LICENSE-CC0` or per-dataset SPDX headers for original project data, per the DATA-LICENSING three-tier policy — deferred at repo setup because no dataset existed then
- [ ] T066 Post-implementation Constitution Check: re-verify all seven gates in `specs/001-venue-session-state/plan.md` against the built system
- [ ] T067 Bump `VERSION` and update `CHANGELOG.md` for the first release
