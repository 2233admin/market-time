...
- **Consumers, in priority order**: autonomous agents that trade, then other systems querying programmatically, then people checking a venue's state directly. The ordering is a design constraint rather than positioning — a person can see that a rendered figure looks approximate, an agent cannot. Uncertainty and unknown must therefore be carried in the returned value and made structurally unskippable, not left to rendering. An unknown and a closed market are different values, not one value displayed two ways.
- **The repository ships no venue data**: all three launch venues forbid commercial redistribution of their published schedules and none carves out calendar or factual data (research D6a). Schedules are fetched at run time by the operator under the operator's own relationship with each venue. This makes fetch-at-run-time the only compliant ingestion shape, not one option among several.
