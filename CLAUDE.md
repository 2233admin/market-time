...
1. **Read the constitution**: [`CONSTITUTION.md`](CONSTITUTION.md). It is ratified governance,
...
2. **Read `DATA-LICENSING.md`** before touching any upstream source. Code license does not
   extend to rule data.
3. **Follow OpenSpec**: proposals, design, tasks, and spec deltas live under `openspec/`.
   Skills and commands are generated by `openspec init` and are not committed — run
   `npx @fission-ai/openspec@1.6.0 init --tools claude,codex` once per clone.
